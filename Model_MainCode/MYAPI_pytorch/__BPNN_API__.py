...
def __BPNN_API__(train_X, train_Y, test_X, parameters):
    """
    ...
    2.加载模型和优化器
    '''
    model = MyModel(input_dim=np.size(train_X, 1),
                    output_dim=parameters["outputdim"],
                    dim1=128,
                    dim2=64).to(device)  # 定义模型参数
    print("\n", model)
...
def __load_bpnnmodel__(test_X, outputdim, modeladdress):
    model = MyModel(input_dim=np.size(test_X, 1), output_dim=outputdim,
                    dim1=128, dim2=64)  # 定义模型参数
    model.load_state_dict(torch.load(modeladdress))
    print(model)

    test_X = torch.Tensor(test_X)
    y_predict = model(test_X)
    print(np.size(test_X, 1), len(test_X), len(y_predict))
